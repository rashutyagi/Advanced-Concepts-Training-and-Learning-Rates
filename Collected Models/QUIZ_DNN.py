...
class DNN_Quiz_Net(nn.Module):
  
  def __init__(self):
    super(DNN_Quiz_Net, self).__init__()
    ...
    # adding these conversion block because we need same number of channels before adding hence using 1*1 for this purpose
    self.conversion_block1 =nn.Sequential(
        nn.Conv2d(in_channels=3, out_channels=32, kernel_size=(1,1),bias=False, padding=1, padding_mode='same')) 
    ...
    self.trans_block1= nn.Sequential(nn.MaxPool2d(2,2))
    ...
    # adding these conversion block because we need same number of channels before adding hence using 1*1 for this purpose
    self.conversion_block2 =nn.Sequential(
        nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(1,1),bias=False, padding=1, padding_mode='same'))
    ...
    self.trans_block2= nn.Sequential(nn.MaxPool2d(2,2))
    ...
    # adding these conversion block because we need same number of channels before adding hence using 1*1 for this purpose
    self.conversion_block3 =nn.Sequential(
        nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(1,1),bias=False, padding=1, padding_mode='same'))
    ...
    self.fc=nn.Conv2d(in_channels=128, out_channels=10, kernel_size=(1,1),bias=False, padding_mode='same')
    
  def forward(self, x1):
    x2=self.conv_block1(x1)
    self.x1=self.conversion_block1(x1)
    x3=self.conv_block2(x1+x2)
    x4=self.trans_block1(x1+x2+x3)
    x5=self.conv_block3(x4)
    x4=self.conversion_block2(x4)
    x6=self.conv_block4(x4+x5)
    x7=self.conv_block5(x4+x5+x6)
    x8=self.trans_block2(x5+x6+x7)
    x9=self.conv_block6(x8)
    x8=self.conversion_block3(x8)
    x10=self.conv_block7(x8+x9)
    x11=self.conv_block8(x8+x9+x10)
    x12=self.gap12(x11)
    x13=self.fc13(x12)
    x13=x13.view(-1,10)
